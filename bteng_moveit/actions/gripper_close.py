...
class GripperCloseNode(MoveItActionNode):
    ...
    def provided_ports():
        return [
            ...
            # close_position="0.04"/> would otherwise hand the node the
            # string "0.04" and abort the process in a float field.
            InputPort("close_position", type_hint=float),
    ...
    def make_goal(self):
        ...
        position = float(self.get_input("close_position"))
        joint_tolerance = float(self.get_input("joint_tolerance", 0.001))

        if not joint_names:
            raise ValueError("GripperCloseNode: 'joint_names' port is required")
